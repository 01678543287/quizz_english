...
// Lưu đáp án đã chọn cho từng câu
const STORAGE_KEY = 'quiz_user_answers_duoclieu_v2';
...
function createShuffledOptions() {
    shuffledOptions = quizData.map(q => {
        const options = [
...
// Khởi tạo dropdown chọn chương ở trang chính
function initMainChapterSelect() {
    const mainSelect = document.getElementById('chapter-select-main');
    ...
    mainSelect.appendChild(optionAll);
    ...
    mainSelect.addEventListener('change', (e) => {
        ...
        initMainChapterSelect(); // Cập nhật lại dropdown
    });
}

initMainChapterSelect();
renderPage(currentPage);
